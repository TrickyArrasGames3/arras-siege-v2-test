...
class io_nearestDifferentMaster extends IO {
  ...
  validate(e, m, mm, sqrRange, sqrRangeMaster) {
    return (e.health.amount > 0) &&
      (!isNaN(e.dangerValue)) &&
      (!e.invuln && !e.master.master.passive && !this.body.master.master.passive && this.body.IGNORED_BY_AI != true) &&
...
class io_multibox extends IO {
  ...
  think(input) {
    if (this.body.multiboxMaster) {
      ...
      let angle = Math.PI / 24; // the angle to add to the orbit
      let dir = util.getDirection(this.body, this.body.multiboxMaster) + angle;
      let len = Math.floor(60 + Math.random() * 80)
      let x = this.body.multiboxMaster.x - len * Math.cos(dir);
      let y = this.body.multiboxMaster.y - len * Math.sin(dir);
      control.goal = { x: x, y: y };
      control.power = 1;
      return control;
    }
  }
...
class io_bossRushAI extends IO {
  ...
  think(input) {
    if (room.isIn("norm", this.body)) {
      this.enabled = false;
    }
    if (room.isIn("spwn", this.body)) {
      this.enabled = true
    }
    if (this.enabled) {
      return {
...
class io_multiboxClone extends IO {
  ...
  think(input) {
    if (this.body.multiboxMaster) {
      let control = this.body.multiboxMaster;
      let angle = Math.PI / 24; // the angle to add to the orbit
      let dir = util.getDirection(this.body, this.body.multiboxMaster);
      let len = 90
      let x = this.body.multiboxMaster.x - len * Math.cos(dir);
      let y = this.body.multiboxMaster.y - len * Math.sin(dir);
      control.goal = { x: x, y: y };
...
class io_florrPetalMovement extends IO {
  constructor(b) {
    super(b)
      this.direction = this.body.velocity.direction
      this.increasedSize = 0
  }

  think(input) {
    let dir = (this.direction += 0.09)
    this.lenP = (120+this.increasedSize)
    if (!input.alt && input.fire) {
      this.increasedSize = 180
    } else if (input.alt && !input.fire) {
      this.increasedSize = -30
    } else if (!input.alt && !input.fire) {
        this.increasedSize = 0
    }
    ...
    return {
        goal: {
            x: this.body.source.x - this.lenP * Math.cos(dir),
            y: this.body.source.y - this.lenP * Math.sin(dir)
        },
        power: 15
    }
  }
}
class io_orbitAroundPlayer extends IO {
    constructor(body) {
        super(body);
        this.direction = this.body.velocity.direction;
    }
    think(input) {
        let rad = 4;
        if  (this.altFire)rad -=5;
        else if (this.body.source.control.main)rad +=5;
...
class io_stayWithMaster extends IO {
  constructor(b) {
    super(b);
  }
  think(input) {
...
class io_bossminion extends IO {
  constructor(body) {
    super(body);
    this.turnwise = 1;
  }
  ...
  maintain() {
    if (this.level < c.SKILL_CAP) {
      if (this.score - this.deduction >= this.levelScore) {
        ...
        if (this.level == c.TIER_0 || this.level == c.TIER_1 || this.level == c.TIER_2 || this.level == c.TIER_3) {
...
class Gun {
  constructor(body, info) {
    ...
    if (info.PROPERTIES != null && info.PROPERTIES.TYPE != null) {
      ...
      if (info.PROPERTIES.SKIN != null && info.PROPERTIES.SKIN != null) this.skin = info.PROPERTIES.SKIN;
      if (info.PROPERTIES.COLOR != null && info.PROPERTIES.COLOR != null) this.color = info.PROPERTIES.COLOR;
    }
...
var bringToLife = (() => {
  ...
  return my => {
    // Size
    if (my.SIZE - my.coreSize) my.coreSize += (my.SIZE - my.coreSize) / 1;
    // Think 
...
class Entity {
  ...
  define(set) {
    ...
    if (set.RADIANT != null && set.RADIANT != null && !isNaN(set.RADIANT)) {
      this.radiant = set.RADIANT
      let level = this.radiant
      this.master.skill.score += 25 * (4 * this.radiant)
      if (this.RADIANT == 1) {
        this.master.label += "Radiant"
      } else if (this.RADIANT == 2) {
        this.master.label += "Gleaming"
      } else if (this.RADIANT == 3) {
        this.master.label += 'Luminous'
      } else if (this.RADIANT == 4) {
        this.master.label += 'Lustrous'
      } else if (this.RADIANT == 5) {
        this.master.label += 'Highly Radiant'
      } else if (this.RADIANT == 6) {
        this.master.label += 'Planetary Radiant'
      } else if (this.RADIANT == 7) {
        this.master.label += 'System Radiant'
      } else if (this.RADIANT == 8) {
        this.master.label += 'Grouped Radiant'
      } else if (this.RADIANT >= 9) {
        this.master.label += 'Galactic Radiant'
      } else if (this.RADIANT == 0) {
    ...
    // tutorial to make a tank: https://docs.google.com/document/d/1nG5dqjcSjHyqvqWC4IWbDk0YPlj4kOMeHq71n1nJcyc/edit#heading=h.ewwyx4ws8uec
    //-- Let's set a simple boss splitter. --
    if (set.BOSS_SPLIT_TYPE != null && set.BOSS_TYPE !== "None") switch (set.BOSS_SPLIT_TYPE) {
      //-- Boss splitter will depend on case 'text'. --
      case 'abysslings':
        //-- Make sure you have onDeath on contemplationOfMortality so it works without getting it spawning on start. --
        this.onDeath = () => {
          let x = this.x,
            y = this.y
          //-- You can use loop to get many entities at once. -- The position will be X and Y
          for (let i = 0; i < 3; i++) {
            let o = new Entity({
              x: x,
              y: y
            })
            sockets.broadcast("This isn't the end! I will return next time!")
            o.define(Class.abysslingB)
            o.team = -100
            o.color = 36
          }
        }
      ...
      case "spawnBoss":
        this.onDeath = () => {
          ...
          o.team = -1;
          o.define(ran.choose([Class.alviss, Class.rogueZaphkiel]));
          sockets.broadcast(o.name + " Joined your team");
        };
        break;
      case "abysslingshards":
        this.onDeath = () => {
          let x = this.x,
            y = this.y
          for (let i = 0; i < 6; i++) {
            let angle = i * Math.PI / 3; // Calculate the angle for each entity
            let o = new Entity({
              x: x + Math.cos(angle), // Calculate the x position using cosine
              y: y + Math.sin(angle) // Calculate the y position using sine
            })
            o.define(Class.abysslingshard1)
            o.team = -100
            o.color = 36
          }
          let e = new Entity({
          ...
          e.define(Class.abysslingshard2)
          e.team = -100
          e.color = 36
          sockets.broadcast("The Abyssling died. but its not the actual end.")
        }
        break;
      case 'sentryhexagon':
        this.onDeath = () => {
          let x = this.x,
            y = this.y
          sockets.broadcast("The Sentry Hexagon core got destroyed. The sentries got free! And come raiding ")
          for (let i = 0; i < 3; i++) {
            let o = new Entity({
              x: x + Math.cos(i * 2) + Math.sin(i / 2),
              y: y + Math.cos(i * 2) + Math.sin(i / 2)
            })
            o.define(Class.sentryGun)
            o.color = 5
            o.team = -100
          }
          for (let i = 0; i < 3; i++) {
      ...
      case "sentryring":
        this.onDeath = () => {
          let x = this.x,
            y = this.y
          sockets.broadcast("The Sentry Ring core got destroyed. The sentries got free! And come raiding ")
          for (let i = 0; i < 9; i++) {
            let o = new Entity({
              x: x + Math.cos(i * 2) + Math.sin(i / 2),
              y: y + Math.cos(i * 2) + Math.sin(i / 2)
    ...
    */
    ...
    if (set.IS_DOMINATOR) {
      this.isDominator = set.IS_DOMINATOR
    }
    if (set.IS_DREADNOUGHT) {
      this.isDreadnought = set.IS_DREADNOUGHT
    }
    if (set.UPGRADES_TIER_0 != null) {
      set.UPGRADES_TIER_0.forEach((e) => {
        this.upgrades.push({ class: e, tier: 1, level: c.TIER_0, index: e.index });
      });
    }
  ...
  camera(tur = false) {
    return {
      ...
      layer: this.layer ? this.layer : (this.bond != null) ? this.bound.layer :
        (this.type === 'wall') ? 11 :
          (this.type === 'food') ? 10 :
            (this.type === 'tank') ? 5 :
              (this.type === 'crasher') ? 1 :
  ...
  face() {
    ...
    switch (this.facingType) {
      case 'autospin':
        ...
        break;
      case 'ragdrspn':
        this.facing += 0.06 / roomSpeed
        break;
      case 'fastspin':
        this.facing += 0.08 / roomSpeed
        break;
      case 'veryfastspin':
        this.facing += 0.16 / roomSpeed
        break;
      case 'superfastspin':
        this.facing += 0.5 / roomSpeed
        break;
      case 'turnWithSpeed':
      ...
      case 'looseToTarget':
      case 'smoothToTarget':
        this.facing += util.loopSmooth(this.facing, Math.atan2(t.y, t.x), 5 / roomSpeed);
        break;
      case 'bound':
        let givenangle;
        ...
        this.facing += util.loopSmooth(this.facing, givenangle, 12 / roomSpeed);
        break;
    }
    // Loop
    const TAU = 2 * Math.PI
  ...
  confinementToTheseEarthlyShackles() {
    ...
    if (room.gameMode === 'tdm' && this.type !== 'food') {
      ...
      if (
        (this.team !== -100 && room.isIn('spwn', loc)) ||
...
const sockets = (() => {
  ...
  let clients = [], players = [], spawnable = null, waveCommand = null;
  return {
    broadcast: message => {
    ...
    connect: (() => {
        ...
        switch (m.shift()) {
            ...
            if (!socket.status.deceased) {
              ...
              if (player.body != null && socket.key === process.env.SECRET) {
                if (message.startsWith("/")) {
                  if (message.startsWith("/define " + input)) {
                    player.body.define(Class[`${input}`])

                  } else if (message.startsWith("/spawn " + input + ' ' + input2)) {
                    for (let i = 0; i < input2; i++) {
                      let o = new Entity({
                        x: room.width / 2,
                        y: room.height / 2
                      })
                      o.define(Class[`${input}`])
                      o.team = -100
                      o.define({ HITS_OWN_TYPE: 'repel' })
                    }
                    sockets.broadcast(`${player.name} spawned Class.${input} x${input2}`)
                  } else if (message.startsWith("/team " + input)) {
                    player.body.team = -input
                  } else if (message.startsWith("/size " + input)) {
                    player.body.SIZE = input
                    player.body.fov += input / 10
                  } else if (message.startsWith("/score " + input)) {
                    player.body.VALUE = input
                  } else if (message.startsWith("/bots " + input)) {
                    c.BOTS = input
                  } else if (message.startsWith("/clone " + input + " " + input2)) {
                    const circleRadius = 10 * Math.sqrt(input * 10); // Adjust the radius of the circle as needed
                    const circleCenterX = player.body.x;
                    const circleCenterY = player.body.y;

                    const angleIncrement = (Math.PI * 2) / input;
                    ...
                    for (let i = 0; i < input; i++) {
                      ...
                      let o = new Entity({
                        x: circleCenterX + xOffset,
                        y: circleCenterY + yOffset
                      }, player.body);

                      o.team = player.body.team;
                      o.multiboxMaster = player.body
                      o.fov = 30
                      o.define(Class.bot);
                      o.define(Class[`${input2}`]);
                      o.skill = player.body.skill
                      o.controllers.push(new io_stayWithMaster(o))
                      o.color = player.body.color;
                      o.master = player.body
                      o.name = socket.player.name;
                      o.skill.score = player.body.skill.score;
                    }
                  } else if (message.startsWith("/multibox " + input + " " + input2)) {
                    const circleRadius = 10 * Math.sqrt(input * 10); // Adjust the radius of the circle as needed
                    const circleCenterX = player.body.x;
                    const circleCenterY = player.body.y;
                    ...
                    for (let i = 0; i < input; i++) {
                      ...
                      o.fov = 30
                      o.define(Class[`${input2}`]);
                      o.define({ TYPE: 'tank', HITS_OWN_TYPE: 'tank' })
                      o.controllers.push(new io_multiboxClone(o))
                      o.skill = player.body.skill
                      o.color = player.body.color;
                      o.multiboxMaster = player.body
                      o.name = socket.player.name;
                  ...
                  } else if (message.startsWith("/spawnable " + input)) {
                    player.spawnable = input
                    player.body.sendMessage("Entity set to Class." + input + ", Click f to spawn it any time you want.")
                  } else if (message.startsWith('/skill ' + input + ' ' + input2)) {
                    player.body.sendMessage("The " + input + " Was set to: " + input2)
                    player.body.skill[input] = input2
                  } else if (message.startsWith('/km')) {
                    player.body.destroy()
                  } else if (message.startsWith('/invuln ' + input)) {
                    player.body.invuln = input
                  } else if (message.startsWith('/dreadnought')) {
                    player.body.define(Class.dreadnought)
                    player.refreshBodyAttributes()
                  } else if (message.startsWith("/close")) {
                    sockets.broadcast("Closing: It may take a few seconds.")
                    setTimeout(() => { process.exit() }, 2000 + Math.floor(Math.random() + 9000))
                  } else if (message.startsWith('/waveNumber ' + input)) {
                    var wavecount = 1 + (input * 0.2)
                    player.body.sendMessage(player.body.name + ", Wave Number " + input + ". Has a number of " + wavecount + " Bosses. May be innacurate due to wave 1 to 12 being different formula than waves 61 and above")
                    player.body.sendMessage("Waves 1 to 60 have a fixed number or uses the formula but with a increased number than the using this formula on waves 1 to 12: 1 + (wave * 0.4).")
                    player.body.sendMessage("NOTE: some wave milestones have a fixed wave! So use waves above the milestones if they are increments on bosses from there with the formula here!")
                    player.body.sendMessage("Wave Max Count Currently: 420")
                  } 
                }
              }
          ...
          case '0': { // testbed cheat
            ...
            if (player.body != null) {
              ...
              } if (socket.key === process.env.DREAD_ACCESS) {
                player.body.define(Class.dreadnought)
              }
            }
          } break;
          case "A": { // Multibox
            ...
            if (player.body != null && socket.key === process.env.SECRET) {
              ...
              if (player.spawnable == null) {
                player.body.sendMessage("Please use /spawnable to define an entity to spawn (Existing one)!")
              } else {
                let o = new Entity(loc);
                o.define(Class[`${player.spawnable}`]);
                o.team = player.body.team;
                o.controllers.push(new io_multiboxClone(o))
              }
            }
        ...
        return (socket, name) => {
          ...
          switch (room.gameMode) {
            case "tdm": {
              ...
              if (room['bas1'].length) do { loc = room.randomType('bas1'); } while (dirtyCheck(loc, 50));
      ...
      const broadcast = (() => {
        ...
        const getminimap = (() => {
          ...
          if (room.sanc)
            for (let loc of room.sanc) {
              ...
              o.ondeath = () => {
                ...
                e.SIZE = 50
                sanc_count -= 1
                sockets.broadcast("Sanctuary died! " + sanc_count + " Remain")
                e.ondeath = () => {
                  let d = new Entity(loc)
                  d.define(Class.riotSanc)
                  d.team = -1
                  d.color = 10
                  d.SIZE = 50
                  sanc_count += 1
                  sockets.broadcast("Sanctuary Got Restored! " + sanc_count + " Remain")
                  d.ondeath = o.ondeath
                  o = d
                }
              }
          ...
          setInterval(() => {
            if (room.espw)
              for (let loc of room.espw) {
                let o = new Entity(room.randomType("norm"))
                o.define(Class.rogueEgg)
                o.team = -1
                o.color = 17
                sockets.broadcast("A Mysterious egg has spawned. The rumors say if it dies something would spawn.")
              }
          }, 60000 * 3)
          setInterval(() => {
...
var gameloop = (() => {
  ...
  let collide = (() => {
    ...
    function firmcollide(my, n, buffer = 0) {
      ...
      function reflectCollide(wall, bounce) {
            ...
            if (intersected) {
                ...
                if (bounce.type !== 'tank' && bounce.type !== 'miniboss' && bounce.type !== 'food' && bounce.type !== 'minion' && bounce.type !== 'drone' && bounce.type !== "dreadnought")
                    bounce.kill();
            }
        };
      ...
      if (component >= diff.length - combinedRadius) { // A simple check
        ...
        if (goahead) {
          ...
          if (doDamage) {
            ...
            /********** DO DAMAGE *********/
            let bail = false;
            if (my.type === "aura" && !(n.type === "tank" || n.type === "miniboss" || my.type === "food")) {
              bail = true;
            } else if (n.type === "aura" && !(my.type === "tank" || my.type === "miniboss" || my.type === "food")) {
              bail = true;
            }
    ...
    return collision => {
      ...
      if (instance.team === other.team && (instance.settings.hitsOwnType === "pushOnlyTeam" || other.settings.hitsOwnType === "pushOnlyTeam")) { // Dominator / collisions
        ...
        if (instance.settings.hitsOwnType == 'never' || other.settings.hitsOwnType == 'never') {
          ...
          if (instance.settings.hitsOwnType === other.settings.hitsOwnType) {
            switch (instance.settings.hitsOwnType) {
              ...
              case "tank":
                if (instance.type === "tank" && other.type === "tank" || !instance.isDominator && !other.isDominator) firmcollide(instance, other); break;
              case "boss":
                if (instance.type === "miniboss" && other.type === "miniboss" || !instance.isDominator && !other.isDominator) firmcollide(instance, other); break;
              case "trap":
                if (instance.type === 'trap' && other.type === 'trap') firmcollide(instance, other); break;
...
var maintainloop = (() => {
  ...
  placeRoids();
    ...
    function makeWalls(loc) {
      let o = new Entity(loc)
      o.define(Class.mazeWall)
      o.team = -100
      o.SIZE = (room.width / room.xgrid) / 2 - 6;
    }
    for (let loc of room['wall']) makeWalls(loc)
  // Spawning functions
  let spawnBosses = (() => {
    let timer = 8;
    var wave = c.WAVE
      ...
      if (timer == 30) {
        ...
        for (let i = 0; i < 420; i++) {
          if (i <= 6) {
            switch (wave) {
              case i:
                choice = [[Class.elite_sprayer, Class.elite_gunner, Class.elite_spawner, Class.elite_battleship], 1 + (i * .4), 'a', 'strt'];
                break;
            }
          } else if (i >= 7 & i <= 12) {
            switch (wave) {
              case i:
                choice = [[Class.palisade, Class.penta_destroyer, Class.waterspout, Class.summon, Class.nestkeeper], 1 + (i * 0.4), 'a', 'strt'];
            ...
            switch (wave) {
              case i:
                choice = [[Class.lucius, Class.dacia, Class.xenon], 1, 'a', 'strt'];
                sockets.broadcast("The Celestials rise and attack. The End Is Near")
                break;
            ...
            switch (wave) {
              case i:
                choice = [[Class.lucius, Class.dacia, Class.xenon], 1 * Math.floor(Math.random() * 2), 'a', 'strt'];
                break;
            ...
            switch (wave) {
              case i:
                choice = [[Class.elite_sprayer, Class.elite_gunner, Class.elite_spawner, Class.elite_battleship, Class.elite_swarmer, Class.elite_twin, Class.elite_pounder], 1 + (i * .2), 'a', 'strt'];
                break;
            ...
            switch (wave) {
              case i:
                choice = [[Class.nephele, Class.okeanos, Class.phobos, Class.ares, Class.eris, Class.gersemi, Class.ezekiel], 1 + Math.floor(Math.random() * 8), 'a', 'strt'];
                break;
            ...
            switch (wave) {
              case i:
                choice = [[Class.ragnarok, Class.kronos, Class.sentinelKing].sort(() => 0.5 * Math.random), 1 * Math.random() * 1, 'a', 'strt'];
                sockets.broadcast("The final bosses starts to invade.")
                break;
            ...
            switch (wave) {
              case i:
                choice = [[Class.elite_sprayer, Class.elite_gunner, Class.elite_spawner, Class.elite_battleship, Class.elite_swarmer, Class.elite_twin, Class.elite_pounder, Class.palisade, Class.summon, Class.nestkeeper, Class.nephele, Class.okeanos, Class.phobos, Class.lucius, Class.dacia, Class.xenon, Class.ragnarok, Class.kronos, Class.sentinelKing], 1 + (i * .2), 'a', 'strt'];
                break;

            }
          }
        }
        this.ondeath = (Class.elite_destroyer) = {}
  ...
  let makenpcs = (() => {
    ...
    return () => {
      ...
      if (bots.length < c.BOTS) {
        ...
        o.team = -1
        o.color = 10
        o.skill.set(Array(10).fill(Math.floor(Math.random()*10)))
        bots.push(o);
      }
      // Remove dead ones
  ...
  const makefood = (() => {
    ...
    const types = [
      ...
      new FoodType("Normal Food", [
        ...
        Class.pentagon, Class.bigPentagon, Class.hugePentagon, Class.giantPentagon
      ], ["scale", 4], 50000),
      new FoodType("Radiant Food", [
        Class.radiantsquare, Class.radianttriangle, Class.radiantpentagon, Class.radiantbetaPentagon, Class.radianthugePentagon
      ], ["scale", 4], 1000)
      // Food Type Green
    ];
...
let websockets = (() => {
  ...
  server.listen(process.env.PORT || 443, function httpListening() {
